...
for dir in \
  ...
  "$DIR/../../build/Windows/master/release/bin"
do
  if test -x "$dir/xwift-daemon"
  then
    xwift-daemon="$dir/xwift-daemon"
    break
  fi
done
if test -z "$xwift-daemon"
then
  echo "xwift-daemon not found"
  exit 1
fi
...
if test "$ready" = 0
then
  echo "Error starting xwift-daemon"
