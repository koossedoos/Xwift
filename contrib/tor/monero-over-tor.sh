...
if test "$ready" = 0
then
  echo "Error starting xwiftd"
  tail -n 400 "$HOME/.xwift/xwift.log" | grep -Ev stacktrace\|"Error: Couldn't connect to daemon:"\|"src/daemon/main.cpp:.*Xwift\ \'" | tail -n 20
  exit 1
fi
